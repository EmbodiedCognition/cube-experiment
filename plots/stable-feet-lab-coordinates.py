...
def main(root, pattern='*'):
    ...
    with lmj.plot.axes(aspect='equal') as ax:
        ...
        for lm, left, rm, right in load(root, pattern):
            ...
            ax.scatter(left.x, left.z, s=40, alpha=0.5, color='#ff7f0e', label=legend and 'Left Foot')
            ax.scatter(right.x, right.z, s=40, alpha=0.5, color='#1f77b4', label=legend and 'Right Foot')
            legend = None
